...
def get_args():
  ...
  # add arguments here
  help_msg = "Source directory or audio file path for all audio files."
  parser.add_argument("-s", "--src", type=str, help=help_msg)

  help_msg = "Write metadata to Excel (.xlsx) file."
  parser.add_argument("-x",
                      "--write-to-excel",
                      action="store_true",
                      help=help_msg)

  help_msg = "Given an Excel (.xlsx) metadata file containing tags and the " \
             "audio file paths of the files associated, write tags to the " \
             "files."
  parser.add_argument("-t", "--tag-file", action="store_true", help=help_msg)

  help_msg = "Modifiers to apply to metadata."
  parser.add_argument("-m", "--modifier", type=str, help=help_msg)

  help_msg = "Clears the tags for a given directory. (`all` clears all tags " \
             "and `excess` clears all tags not in desired base metadata). "
  parser.add_argument("--clear-tags", type=str, help=help_msg)

  help_msg = "Copies the audio file from the source path to destination path."
  parser.add_argument("-c", "--copy-file", action="store_true", help=help_msg)

  help_msg = "Base destination directory for file output (e.g. from file " \
             "renaming or playlist generation)."
  parser.add_argument("-d", "--dst", type=str, help=help_msg)

  help_msg = "Commit changes to audio file."
  parser.add_argument("-w",
                      "--write-to-file",
                      action="store_true",
                      help=help_msg)

  help_msg = "Create configuration file for the application."
  parser.add_argument("--generate-config", action="store_true", help=help_msg)

  help_msg = "Creates a playlist from the given query on the source."
